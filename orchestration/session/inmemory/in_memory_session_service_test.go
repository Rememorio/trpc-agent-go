...
func TestUpdateAndDeleteSessionSummary(t *testing.T) {
	service := NewSessionService()
	ctx := context.Background()
	key := session.Key{
		AppName:   "testapp",
	...
	err = service.UpdateSessionSummary(ctx, key, summary)
	assert.NoError(t, err)

	// Get session and check summary.
	sess2, err := service.GetSession(ctx, key)
	assert.NoError(t, err)
	assert.Equal(t, summary, sess2.Summary)

	// Update summary again.
	newSummary := "Updated summary."
	err = service.UpdateSessionSummary(ctx, key, newSummary)
	assert.NoError(t, err)
	sess3, err := service.GetSession(ctx, key)
	assert.NoError(t, err)
	assert.Equal(t, newSummary, sess3.Summary)

	// Delete summary.
	err = service.DeleteSessionSummary(ctx, key)
	assert.NoError(t, err)
	sess4, err := service.GetSession(ctx, key)
	assert.NoError(t, err)
	assert.Equal(t, "", sess4.Summary)

	// Try to update summary for non-existent session.
	nonExistKey := session.Key{
		AppName:   "testapp",
		UserID:    "user2",
		SessionID: "notfound",
	}
	err = service.UpdateSessionSummary(ctx, nonExistKey, "should not fail")
	assert.NoError(t, err)

	// Try to delete summary for non-existent session.
	err = service.DeleteSessionSummary(ctx, nonExistKey)
	assert.NoError(t, err)
}
