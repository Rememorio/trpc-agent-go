...
| `WithTimeThreshold(interval time.Duration)` | Evaluated during summary checks; triggers when the latest unsummarized event is older than the interval |
...
| `CheckTimeThreshold(interval time.Duration)` | Returns true when the latest unsummarized event is older than the interval (evaluated only when a summary check runs) |
...
- On a summary check, the latest unsummarized event is older than the interval (`WithTimeThreshold`)
- Custom combined conditions met (`WithChecksAny` / `WithChecksAll`)

`WithTimeThreshold` is not a standalone background timer. The condition is only evaluated when a summary check runs, typically after a conversation turn completes or when you call summary APIs manually. For example, `5*time.Minute` means "on the next summary check, if the latest unsummarized event is already older than 5 minutes, summarize now."
