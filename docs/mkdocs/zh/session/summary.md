...
| `WithTimeThreshold(interval time.Duration)` | 在执行摘要检查时，如果最近一个待摘要事件距离当前已超过该间隔，则触发 |
...
| `CheckTimeThreshold(interval time.Duration)` | 在执行摘要检查时，如果最近一个待摘要事件距离当前已超过该间隔，则返回 true |
