...
>
> 作为补充检索路径，Session Recall 进一步把总体 F1 推到
> **0.483**，同时将 Tokens/QA 降到 **2,794**。相比
> Long-Context，token 成本降低 **85.1%**；相比优化版，
> 降低 **83.7%**。
...
| --- | ---: | ---: | ---: | ---: |
| 5 类加权 (÷1986) | 0.469 | **0.483** | 0.469 | 0.399 |
| 4 类加权 (÷1540，排除 adversarial) | 0.411 | **0.505** | 0.423 | 0.279 |
...
>
> 作为补充方案，Session Recall 改变了整体权衡：它在
> **multi-hop** 和 **open-domain** 上表现最佳，将 4 类加权 F1
> 推到 **0.505**。优化版依然在 **temporal** 和
> **adversarial** 上更强。
...
| **平均** | **199** | 0.469 | **0.483** | 0.469 | 0.399 |

> 优化版相较原版在全部 10 个样本上都有提升，并在其中 6 个样本上
> 超过 Long-Context。
>
> 作为补充方案，Session Recall 在 10 个样本里有 6 个超过
> Long-Context，有 7 个超过优化版，提升最大的样本是
> `locomo10_2`、`locomo10_3` 和 `locomo10_10`。
...
| **总体 F1** | 0.469 | **0.483** | 0.469 |
| **4 类加权 F1** | 0.411 | **0.505** | 0.423 |
| **Tokens/QA** | 18,776 | **2,794** | 17,182 |
| **突出优势** | adversarial 更稳 | open-domain / multi-hop 最强 | temporal / adversarial 更均衡 |
...
| 维度 | trpc (session recall) | trpc (优化版) | AutoGen | CrewAI | ADK | Agno |
| --- | --- | --- | --- | --- | --- | --- |
...
| Tokens/QA | 2,794（有效 2,747†） | 17,182（有效 9,663‡） | **1,943** | 2,839 | 49,224 | 10,436 |

> † Session Recall 的 cache 命中率为 1.7%，实际*新增* token
> 成本约为 2,747/QA。
>
...
> Session Recall 目前给出了最好的质量/成本平衡，而优化版则仍是
> 更偏抽取式、tool-driven 的另一条路线。
...
> † Session Recall 的 cache 命中率为 1.7%，实际新增 token 成本
> 约为 ~2,747/QA。
>
> ‡ 优化版有 43.9% 的 prompt tokens 命中提供商 prompt
...
| 类别 | Count | trpc (session recall) | trpc (优化版) | AutoGen | CrewAI | trpc (原版) | ADK | Agno |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
| 平均方式 | trpc (session recall) | trpc (优化版) | AutoGen | CrewAI | trpc (原版) | ADK | Agno |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 5 类加权 (÷1986) | **0.483** | 0.469 | 0.457 | 0.427 | 0.399 | 0.362 | 0.332 |
| 4 类加权 (÷1540) | 0.505 | 0.423 | **0.511** | 0.420 | 0.279 | 0.420 | 0.267 |

> 优化版相较原版依然有明显进步，尤其在 **single-hop**、
> **multi-hop** 和 **temporal** 上改善显著；Session Recall
> 应被看作在这一内部演进基础上的补充检索路径。
>
> 5 类加权 F1 方面，**Session Recall 以 0.483 排名第一**，
> 领先优化版（0.469）0.014，领先 AutoGen（0.457）0.026。
> 4 类加权 F1 达到 **0.505**，仅比 AutoGen 的 0.511 低 0.006，
> 但已显著高于其他 trpc-agent-go 方案和专用记忆系统。
...
> 折中**：它以 2,794 tokens/QA 达到 0.483 F1，显著低于
> Long-Context 和优化版的 token 成本。优化版因为多步 tool-call
> 模式需要反复重读上下文，名义 token 显著更高；虽然 prompt cache
> 能缓解这部分成本，但在当前配置下 Session Recall 仍然明显更轻量。
> ADK 效率最低——49,224 tokens/QA 仅获得 0.362 的 F1。
...
| 样本 | QA 数 | trpc (session recall) | trpc (优化版) | AutoGen | CrewAI | trpc (原版) | ADK | Agno |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
| **平均** | **199** | **0.483** | 0.469 | 0.457 | 0.427 | 0.399 | 0.362 | 0.332 |

> Session Recall 在 10 个样本中的 8 个上超过 AutoGen。
...
| **trpc-agent (session recall)** | 0.349 | **0.514** | **0.598** | 0.121 | 0.505 | 本工作 |
...
1. **trpc-agent（session recall）** 的 4 类加权 F1 达到
   **0.505**，排名 **第二**，仅比 AutoGen（0.511）低 0.006，
   但显著超过 Mem0g（0.422）、Mem0（0.421）、Zep（0.403）、
   LangMem（0.362）、A-Mem（0.347）等专用记忆系统
2. **open-domain 与 multi-hop 成为突出强项。**
   Session Recall 在 **multi-hop**（0.514）和
   **open-domain**（0.598）上都达到第一
...
1. **trpc-agent-go 的 Session Recall 已成为当前最强配置。**
   它在 **5 类加权 F1 上排名第一**（**0.483**），4 类加权 F1
   达到 **0.505**，仅比 AutoGen 低 0.006。相比 Long-Context
...
3. **trpc-agent-go 已明显超越专用记忆系统。** Session Recall 的
   4 类加权 F1 达到 0.505，显著高于 Mem0g（0.422）、Mem0
...
6. **temporal 仍然是下一步重点优化方向。** 优化版在
   temporal 上达到 0.247，但 Session Recall 目前只有 0.121。
